...
export class KakaoProvider {
      ...
      AppAvailability.check(scheme).then(
          ...
          ()=>{  // Error callback
             // ios doesn't generate event. What can I do here? 
              ...
              this.browserRef=new InAppBrowser("https://kauth.kakao.com/oauth/authorize?client_id="+ConfigProvider.kakaoTakitShop+"&redirect_uri="+ConfigProvider.kakaoOauthUrl+"&response_type=code","_blank");
              ...
              this.browserRef.on("loadstart").subscribe((event:InAppBrowserEvent)=>{
                  ...
                  //console.log("url:"+url);
                  //console.log("compare "+ConfigProvider.kakaoOauthUrl+"?code=");
                  ...
                  if(url.startsWith(ConfigProvider.kakaoOauthUrl+"?code=")){
                      ...
                      this.getKakaoToken(ConfigProvider.kakaoTakitShop,ConfigProvider.kakaoOauthUrl,authorize_code).then(
                          (token:any)=>{ 
  ...
  kakaoServerLogin(kakaoid,kakaoProvider:KakaoProvider){
      return new Promise((resolve, reject)=>{
             ...
             kakaoProvider.http.post(ConfigProvider.serverAddress+"/shop/kakaoLogin",body,{headers: headers}).map(res=>res.json()).subscribe((res)=>{
